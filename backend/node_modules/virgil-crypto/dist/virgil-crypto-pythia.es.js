 ...
 */
function createPythiaWrapper(lib$$1) {
    if (typeof lib$$1.VirgilPythia !== 'function') {
        return null;
    }
    var wrapper = createNativeTypeWrapper(lib$$1);
    wrapper.createSafeInstanceMethods(lib$$1.VirgilPythia, [
        'blind',
    ...
    var createVirgilPythia = function () {
        var pythia = new lib$$1.VirgilPythia();
        return pythia;
    };
    var createVirgilPythiaTransformationKeyPair = function (privateKey, publicKey) {
        var privateKeyArr = wrapper.utils.bufferToVirgilByteArray(privateKey);
        var publicKeyArr = wrapper.utils.bufferToVirgilByteArray(publicKey);
        var keyPair = new lib$$1.VirgilPythiaTransformationKeyPair(privateKeyArr, publicKeyArr);
        return keyPair;
    };
    return {
        blind: function (password) {
            var pythia = createVirgilPythia();
            var result = pythia.blindSafe(password);
            var blindedPasswordArr = result.blindedPassword();
            var blindingSecretArr = result.blindingSecret();
            return {
        ...
        computeTransformationKeyPair: function (transformationKeyId, pythiaSecret, pythiaScopeSecret) {
            var pythia = createVirgilPythia();
            var keyPair = pythia.computeTransformationKeyPairSafe(transformationKeyId, pythiaSecret, pythiaScopeSecret);
            var privateKeyArr = keyPair.privateKey();
            var publicKeyArr = keyPair.publicKey();
            return {
        ...
        deblind: function (transformedPassword, blindingSecret) {
            var pythia = createVirgilPythia();
            return pythia.deblindSafe(transformedPassword, blindingSecret);
        },
        getPasswordUpdateToken: function (oldTransformationPrivateKey, newTransformationPrivateKey) {
            var pythia = createVirgilPythia();
            return pythia.getPasswordUpdateTokenSafe(oldTransformationPrivateKey, newTransformationPrivateKey);
        },
        prove: function (transformedPassword, blindedPassword, transformedTweak, transformationKeyPair) {
            transformationKeyPair = createVirgilPythiaTransformationKeyPair(transformationKeyPair.privateKey, transformationKeyPair.publicKey);
            var pythia = createVirgilPythia();
            var result = pythia.proveSafe(transformedPassword, blindedPassword, transformedTweak, transformationKeyPair);
            var proofValueCArr = result.proofValueC();
            var proofValueUArr = result.proofValueU();
            return {
        ...
        transform: function (blindedPassword, tweak, transformationPrivateKey) {
            var pythia = createVirgilPythia();
            var result = pythia.transformSafe(blindedPassword, tweak, transformationPrivateKey);
            var transformedPasswordArr = result.transformedPassword();
            var transformedTweakArr = result.transformedTweak();
            return {
        ...
        updateDeblindedWithToken: function (deblindedPassword, passwordUpdateToken) {
            if (deblindedPassword == null)
                throw new Error('`deblindedPassword` is required');
            if (passwordUpdateToken == null)
                throw new Error('`passwordUpdateToken` is required');
            var pythia = createVirgilPythia();
            return pythia.updateDeblindedWithTokenSafe(deblindedPassword, passwordUpdateToken);
        },
        verify: function (transformedPassword, blindedPassword, tweak, transformationPublicKey, proofValueC, proofValueU) {
            var pythia = createVirgilPythia();
            return pythia.verifySafe(transformedPassword, blindedPassword, tweak, transformationPublicKey, proofValueC, proofValueU);
        }
    };
}
...
var pythiaWrapper = createPythiaWrapper(lib);
/**
 * Class containing Pythia-related cryptographic operations.
 */
var VirgilPythiaCrypto = /** @class */ (function () {
    function VirgilPythiaCrypto() {
        if (pythiaWrapper === null) {
            throw new Error('Cannot create an insance of `VirgilPythiaCrypto`. ' +
     ...
     */
    VirgilPythiaCrypto.prototype.blind = function (password) {
        var passwordBuf = anyToBuffer(password, 'utf8', 'password');
     ...
     */
    VirgilPythiaCrypto.prototype.deblind = function (params) {
        var transformedPassword = anyToBuffer(params.transformedPassword, 'base64', 'params.transformedPassword');
        var blindingSecret = anyToBuffer(params.blindingSecret, 'base64', 'params.blindingSecret');
     ...
     */
    VirgilPythiaCrypto.prototype.computeTransformationKeyPair = function (params) {
        var transformationKeyId = anyToBuffer(params.transformationKeyId, 'base64', 'params.transformationKeyId');
        var pythiaSecret = anyToBuffer(params.pythiaSecret, 'base64', 'params.pythiaSecret');
        var pythiaScopeSecret = anyToBuffer(params.pythiaScopeSecret, 'base64', 'params.pythiaScopeSecret');
        return pythiaWrapper.computeTransformationKeyPair(transformationKeyId, pythiaSecret, pythiaScopeSecret);
    };
    /**
     ...
     */
    VirgilPythiaCrypto.prototype.transform = function (params) {
        var blindedPassword = anyToBuffer(params.blindedPassword, 'base64', 'params.blindedPassword');
        var tweak = anyToBuffer(params.tweak, 'base64', 'params.tweak');
        var transformationPrivateKey = anyToBuffer(params.transformationPrivateKey, 'base64', 'params.transformationPrivateKey');
     ...
     */
    VirgilPythiaCrypto.prototype.prove = function (params) {
        var transformationKeyPair = {
            privateKey: anyToBuffer(params.transformationKeyPair.privateKey, 'base64', 'params.transformationKeyPair.privateKey'),
            publicKey: anyToBuffer(params.transformationKeyPair.publicKey, 'base64', 'params.transformationKeyPair.publicKey')
        };
        var transformedPassword = anyToBuffer(params.transformedPassword, 'base64', 'params.transformedPassword');
        var blindedPassword = anyToBuffer(params.blindedPassword, 'base64', 'params.blindedPassword');
        var transformedTweak = anyToBuffer(params.transformedTweak, 'base64', 'params.transformedTweak');
     ...
     */
    VirgilPythiaCrypto.prototype.verify = function (params) {
        var transformedPassword = anyToBuffer(params.transformedPassword, 'base64', 'params.transformedPassword');
        var blindedPassword = anyToBuffer(params.blindedPassword, 'base64', 'params.blindedPassword');
        var tweak = anyToBuffer(params.tweak, 'base64', 'params.tweak');
        var transformationPublicKey = anyToBuffer(params.transformationPublicKey, 'base64', 'params.transformationPublicKey');
        var proofValueC = anyToBuffer(params.proofValueC, 'base64', 'params.proofValueC');
        var proofValueU = anyToBuffer(params.proofValueU, 'base64', 'params.proofValueU');
        return pythiaWrapper.verify(transformedPassword, blindedPassword, tweak, transformationPublicKey, proofValueC, proofValueU);
    };
    /**
     ...
     * @returns {Buffer}
     */
    VirgilPythiaCrypto.prototype.getPasswordUpdateToken = function (params) {
        var oldTransformationPrivateKey = anyToBuffer(params.oldTransformationPrivateKey, 'base64', 'params.oldTransformationPrivateKey');
        var newTransformationPrivateKey = anyToBuffer(params.newTransformationPrivateKey, 'base64', 'params.newTransformationPrivateKey');
     ...
     */
    VirgilPythiaCrypto.prototype.updateDeblindedWithToken = function (params) {
        var deblindedPassword = anyToBuffer(params.deblindedPassword, 'base64', 'params.deblindedPassword');
        var updateToken = anyToBuffer(params.updateToken, 'base64', 'params.updateToken');
        return pythiaWrapper.updateDeblindedWithToken(deblindedPassword, updateToken);
    };
    return VirgilPythiaCrypto;
}());

export { KeyPairType, HashAlgorithm, VirgilAccessTokenSigner, VirgilPrivateKeyExporter, VirgilCardCrypto, encoding, VirgilPrivateKey, VirgilPublicKey, VirgilCrypto, VirgilPythiaCrypto };
